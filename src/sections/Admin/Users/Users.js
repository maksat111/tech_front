...
function Users() {
  ...
  const handleOk = async () => {
    try {
      ...
      if (newItem) {
        const res = await axiosInstance.patch(`admin/update/${newItem._id}`, {
        ...
        await axiosInstance.delete(`admin/delete/${selectedItem._id}`);
  ...
  const handleAddOk = async () => {
    ...
    try {
      if (newItem._id) {
        ...
        const res = await axiosInstance.post("admin/auth/register", formData);
        const added = {
  ...
  const handleTableChange = async (a, b, c) => {
    ...
    if (c.field !== ordering?.field || c.order !== ordering?.order) {
      ...
      if (c.order == "ascend") {
        var query = `admin/list?ordering=${c.field}`;
      } else {
        var query = `admin/list?ordering=-${c.field}`;
      }
      axiosInstance
  ...
  useEffect(() => {
    axiosInstance.get(`admin/list?search=${searchValue}`).then((res) => {
      res.data.data?.forEach((element) => {
        element.key = element._id;
      });
      setDataSource(res.data.data);
    ...
    <>
      ...
      >
        <form className="banner-add-container">
          <div className="add-left">
            <div className="add-column">Имя:</div>
            <div className="add-column">Фамилия:</div>
            <div className="add-column">Электронная почта:</div>
            <div className="add-column">Номер телефона:</div>
            <div className="add-column">Пароль:</div>
            {newItem?._id && <div className="add-column">Новый пароль:</div>}
          </div>
      ...
      <Modal
        title={
          !newItem
            ...
            : "Вы уверены, что хотите изменить активност?"
        }
        open={open}
        onOk={handleOk}
        confirmLoading={confirmLoading}
      ...
      <div className="page">
        <div className="page-header-content">
          <h2 className="admin-section-name">Ulanyjylar</h2>
