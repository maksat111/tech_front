...
function Categories() {
    ...
    },
    {
      ...
      render: (_, record) => (
        <img
          ...
          src={"http://127.0.0.1:5000/" + record.image}
          alt={record.name_ru}
        />
      ),
    },
    ...
    try {
      if (newItem._id) {
        const res = await axiosInstance.patch(
          `admin/category/update/${newItem._id}`,
          formData
        );
  ...
  useEffect(() => {
    axiosInstance
      .get(`admin/category/list?search=${searchValue}`)
      .then((res) => {
        res.data.data?.forEach((element) => {
          element.key = element._id;
        });
    ...
    <>
      ...
      >
        <div className="banner-add-container">
          <div className="add-left">
            <div className="add-column">Ady (rus.):</div>
            <div className="add-column">Ady (türkmen.):</div>
            <div className="add-column">Ady (eng.):</div>
            <div className="add-picture">Logo</div>
          </div>
          <div className="add-right">
            ...
            <div className="add-picture">
              {newItem?._id && (
                <img
                  className="brand-image"
                  src={"http://127.0.0.1:5000/" + newItem?.image}
                  alt={newItem?.name_ru}
                />
              )}
              <Upload
      ...
      <div className="page">
        <div className="page-header-content">
          <h2 className="admin-section-name">Kategoriýalar</h2>
          <div className="add-button" onClick={showAddModal}>
            Girizmek
          </div>
        </div>
        <div className="categories-header-filters"></div>
        <TableComponent
